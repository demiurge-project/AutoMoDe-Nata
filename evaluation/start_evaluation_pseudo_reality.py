...
EVALUATION_CONTEXT = "PseudoReality"
...
def submit_chocolate_job(args, instance, controllers, method):
...
def submit_neat_job(args, instance, genomes_dir, method):
     ...
     data = {"jobname": experiment_name,
             ...
             "machine": MACHINE,
             "queue": QUEUE,
             "command": "/home/aligot/AutoMoDe-Harlequin/evaluation/evaluate_neat.py -m %s -n %s -d %s -g %s -r %s -e %s" % (instance, args.expname, method, genomes_dir, args.resultdirectory, EVALUATION_CONTEXT),
             }
     p = Popen("qsub -v PATH", shell=True, stdin=PIPE, stdout=PIPE, close_fds=True)
     (child_stdout, child_stdin) = (p.stdout, p.stdin)
     script = JOB_SCRIPT % data
...
if __name__ == "__main__":
    ...
    for method in list_methods:
        if (method == "Chocolate"):
            mission_file = [f for f in config_files if (f.startswith('chocolate_') and 'pseudo_reality' in f)][0]
            create_automode_jobs(args, method, join(args.controllers, mission_file))
        elif (method == "Harlequin"):
            mission_file = [f for f in config_files if (f.startswith('harlequin_') and 'pseudo_reality' in f)][0]
            create_automode_jobs(args, method, join(args.controllers, mission_file))
        else:
            mission_file = [f for f in config_files if (f.startswith('neat_') and 'pseudo_reality' in f)][0]
