...
function write_connection() {
  ...
  echo "C${STATE}x$CONNECTION  \"--c${STATE}x$CONNECTION \" c   (6,7,8,9,10) | as.numeric(NumConnections$STATE)>$CONNECTION " >> ${TXT_FILE}
  echo "P${STATE}x$CONNECTION  \"--p${STATE}x$CONNECTION \" r   (0,1) | as.numeric(C${STATE}x$CONNECTION) %in% c(6,7,8,9,10) " >> ${TXT_FILE}
  echo "T${STATE}x$CONNECTION  \"--t${STATE}x$CONNECTION \" r   (0,20) | as.numeric(C${STATE}x$CONNECTION)==10 " >> ${TXT_FILE}
  echo "TI${STATE}x$CONNECTION  \"--t${STATE}x$CONNECTION \" r   (0,8) | as.numeric(C${STATE}x$CONNECTION) %in% c(7,8) " >> ${TXT_FILE}
  echo "TA${STATE}x$CONNECTION  \"--t${STATE}x$CONNECTION \" c   (0,1,2) | as.numeric(C${STATE}x$CONNECTION)==6 " >> ${TXT_FILE}
  echo "TB${STATE}x$CONNECTION  \"--t${STATE}x$CONNECTION \" c   (0,1,2,3) | as.numeric(C${STATE}x$CONNECTION)==9 " >> ${TXT_FILE}
  echo "B${STATE}x$CONNECTION  \"--b${STATE}x$CONNECTION \" c   (0,1) | as.numeric(C${STATE}x$CONNECTION) %in% c(7,8,10) " >> ${TXT_FILE}
}
