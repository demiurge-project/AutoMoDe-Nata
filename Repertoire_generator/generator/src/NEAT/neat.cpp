...
double NEAT::linktrait_mut_sig =
    0; // Amount that mutation_num changes for a trait change inside a link
double NEAT::nodetrait_mut_sig =
    0; // Amount a mutation_num changes on a link connecting a node that changed its trait
double NEAT::weight_mut_power = 0; // The power of a linkweight mutation
double NEAT::recur_prob =
    0; // Prob. that a link mutation which doesn't have to be recurrent will be made recurrent
...
double NEAT::recur_only_prob =
    0; // Probability of forcing selection of ONLY links that are naturally recurrent
...
double NEAT::weight_lower_bound =
    -8.0; // initial value that was in the original code in Genome::mutate_link_weights(...).
double NEAT::weight_upper_bound =
    8.0; // initial value that was in the original code in Genome::mutate_link_weights(...).
...
double NEAT::frelu(double activesum) {
    return fmax(0,activesum);
}

double NEAT::oldhebbian(double weight, double maxweight, double active_in, double active_out,
    ...
    if (!(neg)) {
        ...
        // input is high
        delta = hebb_rate * (maxweight - weight) * active_in * (1.0 - active_out) + //"unhebb"
...
double NEAT::hebbian(double weight, double maxweight, double active_in, double active_out,
                     double hebb_rate, double pre_rate, double post_rate) {
