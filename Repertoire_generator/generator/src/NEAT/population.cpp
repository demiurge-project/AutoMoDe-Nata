...
Population::Population(std::vector<Genome*> genomeList, float power) {
    ...
    for (std::vector<Genome*>::iterator iter = genomeList.begin(); iter != genomeList.end();
         ++iter) {
...
Population::~Population() {
    ...
    for (std::vector<Innovation*>::iterator iter = innovations.begin(); iter != innovations.end();
         ++iter)
        delete *iter;
}
    ...
    */
    ...
    if (((*(((*curspecies)->organisms).begin()))->orig_fitness) > highest_fitness) {
        ...
        std::cout << highest_last_changed
                  << " generations since last population fitness record: " << highest_fitness
                  << std::endl;
    }
    ...
    //  the system parameter babies_stolen (when babies_stolen > 0)
        ...
        while ((stolen_babies < NUM_STOLEN) && (curspecies != sorted_species.begin())) {
            ...
            // cout<<"Considering Species "<<(*curspecies)->id<<": age "<<(((*curspecies)->age))<<"
            // expected offspring "<<(((*curspecies)->expected_offspring))<<endl;
        ...
        if ((stolen_babies >= one_fifth_stolen) && (curspecies != sorted_species.end())) {
            ...
            //      cout<<"The best superchamp is
            //      "<<(*(((*curspecies)->organisms).begin()))->gnome->genome_id<<endl;
    ...
    while (curspecies != species.end()) {
        ...
        //(the iterator must be reset because there were possibly vector insertions during
        //reproduce)
    ...
    for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
        if ((*curorg)->pop_champ_child) {
            // cout<<"At end of reproduction cycle, the child of the pop champ is:
            // "<<(*curorg)->gnome<<endl;
        }
    }
