...
Organism* Species::get_champ() {
    ...
    for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
        ...
        // cout<<"searching for champ...looking at org "<<(*curorg)->gnome->genome_id<<" fitness:
        // "<<(*curorg)->fitness<<endl;
...
Organism* Species::first() { return *(organisms.begin()); }
/*
bool Species::print_to_file(std::ostream &outFile) {
        std::vector<Organism*>::iterator curorg;

        //Print a comment on the Species info
        //outFile<<endl<<" Species #"<<id<<" : (Size "<<organisms.size()<<") (AF "<<ave_fitness<<")
(Age "<<age<<")  //"<<endl<<endl;
        //char tempbuf[1024];
        //sprintf(tempbuf, sizeof(tempbuf), "Species #%d : (Size %d) (AF %f) (Age %d)  \n\n", id,
organisms.size(), average_est, age);
        //sprintf(tempbuf, sizeof(tempbuf), " Species #%d : (Size %d) (AF %f) (Age %d)  \n\n", id,
organisms.size(), ave_fitness, age);
        //outFile.write(strlen(tempbuf), tempbuf);

        //Show user what's going on
        //cout<<endl<<" Species #"<<id<<" : (Size "<<organisms.size()<<") (AF "<<ave_fitness<<")
(Age "<<age<<")  "<<endl;
        ...
        //for(curorg=organisms.begin();curorg!=organisms.end();++curorg) {
                ...
                //outFile<<endl<<" Organism #"<<((*curorg)->gnome)->genome_id<<" Fitness:
"<<(*curorg)->fitness<<" Error: "<<(*curorg)->error<<" "<<endl;
...
$------## \n", ((*curorg)->gnome)->genome_id, id);
                        //outFile<<" ##------$ WINNER "<<((*curorg)->gnome)->genome_id<<" SPECIES
#"<<id<<" $------## "<<endl;
        //	}
...
bool Species::print_to_file(std::ofstream& outFile) {
    ...
    // Print a comment on the Species info
    outFile << std::endl
            << "/* Species #" << id << " : (Size " << organisms.size() << ") (AF " << ave_fitness
            << ") (Age " << age << ")  */" << std::endl
            << std::endl;
    ...
    for (curorg = organisms.begin(); curorg != organisms.end(); ++curorg) {
        ...
        // Put the fitness for each organism in a comment
        outFile << std::endl
                << "/* Organism #" << ((*curorg)->gnome)->genome_id
                << " Fitness: " << (*curorg)->fitness << " Error: " << (*curorg)->error << " */"
                << std::endl;
...
bool Species::print_to_file(std::ostream& outFile) {
    ...
    // Print a comment on the Species info
    // outFile<<std::endl<<"/* Species #"<<id<<" : (Size "<<organisms.size()<<") (AF
    // "<<ave_fitness<<") (Age "<<age<<")  */"<<std::endl<<std::endl;
    char tempbuf[1024];
    ...
    // Show user what's going on
    // std::cout<<std::endl<<"/* Species #"<<id<<" : (Size "<<organisms.size()<<") (AF
    // "<<ave_fitness<<") (Age "<<age<<")  */"<<std::endl;
...
void Species::adjust_fitness() {
    ...
    // std::cout<<"Species "<<id<<" last improved "<<(age-age_of_last_improvement)<<" steps ago when
    // it moved up to "<<max_fitness_ever<<std::endl;
    ...
    curorg = organisms.begin();
    (*curorg)->champion = true; // Mark the champ as such, it will be saved unchanged in the next
                                // generation (elitism).
    ...
    while (curorg != organisms.end()) {
        ...
        // std::std::cout<<"marked org # "<<(*curorg)->gnome->genome_id<<" fitness =
        // "<<(*curorg)->fitness<<std::std::endl;
        ++curorg;
    }
}
...
bool Species::reproduce(int generation, Population* pop, std::vector<Species*>& sorted_species) {
    ...
    // NOTE: At this point, only the valid parents are inside the species, all the other organisms
    // have been deleted from the species (see code in Population::epoch).
    ...
    for (count = 0; count < expected_offspring; count++) {
        ...
        // If there is only one organism in the pool, then always mutate
            ...
            // Add sensor: immediately connects a randomly chosen sensor which was previously
            // disconnected to all the outputs at once. See FAQ: This is useful if we start NEAT
            // with some disconnected (inputs) sensors. By default, mutate_add_sensor_prob = 0.0
        ...
        // Otherwise we should mate
            ...
            if (randfloat() < NEAT::mate_multipoint_prob) {
                new_genome = (mom->gnome)
                                 ->mate_multipoint(dad->gnome, count, mom->orig_fitness,
                                                   dad->orig_fitness, outside);
            } else if (randfloat() <
                       (NEAT::mate_multipoint_avg_prob /
                        (NEAT::mate_multipoint_avg_prob + NEAT::mate_singlepoint_prob))) {
                new_genome = (mom->gnome)
                                 ->mate_multipoint_avg(dad->gnome, count, mom->orig_fitness,
                                                       dad->orig_fitness, outside);
            } else {
...
bool NEAT::order_species(Species* x, Species* y) {
    // std::cout<<"Comparing "<<((*((x->organisms).begin()))->orig_fitness)<<" and
    // "<<((*((y->organisms).begin()))->orig_fitness)<<":
    // "<<(((*((x->organisms).begin()))->orig_fitness) >
    // ((*((y->organisms).begin()))->orig_fitness))<<std::endl;
