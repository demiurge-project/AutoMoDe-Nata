 ...
 * @note: To launch your own program: you can use, for example, system("") which is a combination of
...
/**
 * Function that launches in parallel (with MPI) the experiment for each organism in the population
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 4) {
        ...
        std::cerr
            ...
            << std::endl;
        std::cerr
            << "Arg4 (optional): The number (natural number) of processes you want to launch. If "
               "none is specified, or if it's 0 or 1, there will be only one process."
            << std::endl;
        std::cerr << "Arg5 (non-optional if more than 1process!): the name of the binary file to "
                     "launch in parallel."
                  << std::endl;
        std::cerr << "Arg6 (optional): The seed (unsigned integer) used to initialize NEAT. If "
