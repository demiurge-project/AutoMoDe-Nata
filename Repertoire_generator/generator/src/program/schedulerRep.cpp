...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        if (nNum_runs_per_gen > 0) {
            dQualityScore /= nNum_runs_per_gen;
        }
        std::vector<double> vecMedBehaviorCharac =
            geometric_median(vecBehaviorCharac, vectors_distance);
        // Send the fitness to the parent.
        std::cout << "ID" << id << " --> Average quality = " << dQualityScore << std::endl;
        parent_comm.Send(&dQualityScore, 1, MPI::DOUBLE, 0, 1);
        parent_comm.Send(&vecMedBehaviorCharac[0], vecMedBehaviorCharac.size(), MPI::DOUBLE, 0, 1);
        // Removing stuffs
        delete genome;
        delete net;
    }
